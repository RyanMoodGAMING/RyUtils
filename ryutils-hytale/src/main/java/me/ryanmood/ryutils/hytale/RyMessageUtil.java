package me.ryanmood.ryutils.hytale;
...
public class RyMessageUtil implements RyMessagesBase<Player, CommandSender> {
    ...
    @Setter(AccessLevel.PRIVATE)
    public HytaleAudiences audiences;
    @Getter(AccessLevel.PRIVATE)
    ...
    public Component adventureTranslate(Player player, String message) {
        if (HytaleServer.get().getPluginManager().getPlugin(PluginIdentifier.fromString("HelpChat:PlaceholderAPI")) != null) {
            message = PlaceholderAPI.setPlaceholders(player.getPlayerRef(), message);
        }
    ...
    public void sendPlayer(Player player, String message) {
        this.audiences.player(player.getPlayerRef()).sendMessage(this.adventureTranslate(player, message));
    }
    ...
    public void sendConsole(boolean prefix, String message) {
        ...
        this.LOGGER.at(Level.INFO).log(this.stripColours(message));
    }
     ...
     * @param message The message you wish to send to console.
     */
    public void sendConsoleWarning(boolean prefix, String message) {
        if (prefix) message = this.getPrefix() + message;

        this.LOGGER.at(Level.WARNING).log(this.stripColours(message));
    }
     ...
     * @param messages The messages you wish to send to console.
     */
    public void sendConsoleWarning(boolean prefix, String... messages) {
        Arrays.stream(messages).forEach(msg -> this.sendConsole(prefix, msg));
    }
     ...
     * @param messages The messages you wish to send to console.
     */
    public void sendConsoleWarning(boolean prefix, List<String> messages) {
        messages.forEach(msg -> this.sendConsole(prefix, msg));
    }
    ...
    public void broadcast(String message) {
        ...
        universe.getPlayers().forEach(targetRef -> {
            this.sendPlayer(this.getPlayerFromRef(targetRef), message);
        });
        LOGGER.at(Level.INFO).log(this.stripColours(message));
    }
    ...
    public void broadcast(String permission, String message) {
        Universe universe = Universe.get();
        universe.getPlayers().forEach(targetRef -> {
            Player target = this.getPlayerFromRef(targetRef);
...
Universe universe = Universe.get();
        universe.getPlayers().forEach(targetRef -> {
            this.getAudiences().player(targetRef).sendMessage(this.adventureTranslate(player, message));
        });
    }
    ...
    public void sendError(String error) {
        this.sendConsoleWarning(true, this.getBreaker(),
    ...
    public void sendError(String error, boolean disable) {
        ...
        if (disable && this.getPlugin() != null) HytaleServer.get().getPluginManager().unload(this.plugin.getIdentifier());
    }
    ...
    public void sendLongError(String... errors) {
        ...
        this.sendConsoleWarning(true, this.getSupportMessage(),
                this.breaker);
    }
    ...
    public Player getPlayerFromRef(PlayerRef playerRef) {
